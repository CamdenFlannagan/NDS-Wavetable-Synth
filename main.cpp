...
private:
     ...
     * 
     * I keep it around because it's derpiness is endearing to me.
     */
...
public:
    ...
    void exportSFZ() {
        ...
        for (int midi_index = 0; midi_index < 128; midi_index++) {
            char file_name[64];
            sprintf(file_name, "sfz/%03d_%s.wav", midi_index, midi.info[midi_index].name);
 ...
 * 
 * Suppose you've developed a lovely new synth class, CoolSynth
 * 
 * First, go to the private variables and declare a variable like this:
 * LinkedRing<Editor *> coolSynthEditorRing;
 * 
 * Now declare the editors you'd like to use, such as sliders, tables, etc, as well as any variables they need,
 * and make sure to declare the synth itself as well:
 * s16 coolTable[TABLE_LENGTH];
 * Table coolTableEditor;
 * int coolInitialSliderValue = 0;
 * Slider coolSlider;
 * int coolInitialSwitchState = 0;
 * Switch coolSwitch;
 * CoolSynth coolsynth;
 * 
 * Now go to the app constructor and initialize everything like this:
 * coolSynthEditorRing(),
 * coolTableEditor(<provide some description text here>, coolTable),
 * coolSlider(<description text>, coolInitialSliderValue, <provide the maximum value the slider can reach>),
 * coolSwitch(<description text>, coolInitialSwitchState, <provide the number of states you want the switch to have>),
 * coolsynth(<gain>, <sample rate for this synth>, <whatever parameters you put in your synth class's constructor>),
 * 
 * Now go to the code that the App constructor runs and link all of your editors together (in reverse order) like this:
 * coolSynthEditorRing.add(&coolSwitch);
 * coolSynthEditorRing.add(&coolSlider);
 * coolSynthEditorRing.add(&coolTableEditor);
 * 
 * Now go to the bottom of the App constructor's code and add this:
 * synEdPairRing.add(new SynEdPair("COOL SYNTH\n\n", &coolSynthEditorRing, &coolsynth));
 * 
 * Now CoolSynth should be connected to the App. Now when you run this on your DS, you can navigate to Cool Synth!
 */
class App {
public:
    App() :
